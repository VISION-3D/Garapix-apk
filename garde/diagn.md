...
const DiagnosticReal = () => {
  ...
  const [pendingStream, setPendingStream] = useState(null); // Stream en attente avant d'être attaché à la vidéo
  ...
  // Attacher le stream à la vidéo dès que le DOM est prêt
  useEffect(() => {
    if (cameraActive && videoRef.current && pendingStream) {
  ...
  }, [cameraActive, pendingStream, videoRef.current]);
  ...
  const initializeCamera = async () => {
    ...
    try {
      ...
      console.log("Stream reçu :", mediaStream);

      setPendingStream(mediaStream); // On stocke temporairement
      setCameraActive(true);
    } catch (error) {
      console.error("Erreur accès caméra :", error);
      if (error.name === "NotAllowedError") {
        alert("Permission caméra refusée. Activez-la dans les paramètres.");
  ...
  const analyzeWithIA = async () => {
    ...
    try {
      ...
      if (identifyData.results && identifyData.results.length > 0) {
        ...
        console.log('Nom scientifique normalisé :', normalizedScientific);

        // Récupérer toutes les plantes
        const allPlants = getAllPlants();
        console.log('Nombre de plantes disponibles :', allPlants.length);
        ...
        if (matchedPlant) {
          console.log('✅ Plante correspondante trouvée :', matchedPlant.nomCommun);
          plantName = matchedPlant.nomCommun;
        } else {
          console.log('❌ Aucune correspondance trouvée pour :', scientificName);
          plantName = scientificName;
        }
      }

      // Diagnostic optionnel (inchangé)
        ...
        <div className="control-panel">
            ...
            {!preview ? (
              <div className="upload-options">
                ...
                <div className="camera-option modern-card">
                  ...
                  {!cameraActive ? (
                    <button className="btn-modern-primary" onClick={initializeCamera}>
                      <Camera size={18} />
                      Activer la caméra
                    </button>
                  ) : (
                  ...
                  <div className="ia-results">
                    ...
                    {diagnosticIA.plantNet && (
                      <div className="result-section">
                        {diagnosticIA.plantNet.species === 'Service indisponible' ? (
                          <>
                            <p><b>⚠️ Service indisponible</b></p>
                    ...
                    {diagnosticIA.plantDoc && (
                      <div className="result-section">
                        ...
                        <p><strong>Recommandation de traitement :</strong>{" "}
          ...
          <div className="guide-card">
            ...
            <ol className="guide-steps">
              ...
              <li><strong>Recherchez dans notre base</strong> – Cliquez sur le bouton pour accéder aux détails ou lancer une recherche</li>
            </ol>
          </div>
        </div>
              ...
              <div className="details-tab">
                ...
                {plantInfo.recolte && renderExpandableSection(
                  ...
                  <div className="harvest-info">
                    ...
                    <div className="harvest-item"><strong>Rendement:</strong> <span>{plantInfo.recolte.rendement}</span></div>
                  </div>,
                  "harvest"
                )}
